...
function getWeekNumberByDate(date) {
  ...
  const firstMondayDay = 1 + ((8 - firstDate.getDay()) % 7);
  const firstMondayMs = new Date(year, 0, firstMondayDay).getTime();
  const dateMs = date.getTime();
  const timezoneOffset = date.getTimezoneOffset() * 60 * 1000;
  const weekMs = 1000 * 60 * 60 * 24 * 7;
  let weekNumber = (dateMs - firstMondayMs - timezoneOffset) / weekMs + 1;
  if (firstMondayDay > 1) weekNumber += 1;
...
function getNextFridayThe13th(date) {
  ...
  let checked = false;
  do {
    if (!checked && dayOfMonth !== 13) nextDate.setDate(13);
    if (checked || dayOfMonth > 13) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const arr = [];
  const firstDayArr = period.start.split('-');
  const lastDayArr = period.end.split('-');
  const firstDay = new Date(
    firstDayArr[2],
    +firstDayArr[1] - 1,
    firstDayArr[0]
  );
  const lastDay = new Date(lastDayArr[2], +lastDayArr[1] - 1, lastDayArr[0]);
  const i = new Date(firstDay.getTime());
  while (i <= lastDay) {
    for (let j = 1; j <= countWorkDays; j += 1) {
      arr.push(
        `${i.getDate().toString().padStart(2, '0')}-${(i.getMonth() + 1).toString().padStart(2, '0')}-${i.getFullYear()}`
      );
      i.setDate(i.getDate() + 1);
      if (i > lastDay) break;
    }
    for (let z = 1; z <= countOffDays; z += 1) {
      i.setDate(i.getDate() + 1);
      if (i > lastDay) break;
    }
  }
